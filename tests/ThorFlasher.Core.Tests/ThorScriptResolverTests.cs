...
public sealed class ThorScriptResolverTests
{
    ...
    [Fact]
    public void GetSendBuildCommand_ReturnsDisabled_ForCapsuleUpdateWithCapFile()
    {
        var scriptsRoot = CreateScriptsRoot();
        var resolver = CreateResolver(scriptsRoot);
        var capFile = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid()}.cap");
        File.WriteAllText(capFile, "test");
        ...
        try
        {
            var context = new OperationContext
            {
                ...
                TargetPassword = "1111",
                FilePath = capFile,
                OperationType = OperationType.CapsuleUpdate,
                ScriptsRootPath = scriptsRoot,
                WorkingDirectory = scriptsRoot
            };
    ...
    [Fact]
    public void GetSendBuildCommand_RemainsEnabled_ForCapsuleUpdateWithBinFile()
    {
        var scriptsRoot = CreateScriptsRoot();
        var resolver = CreateResolver(scriptsRoot);
        var context = CreateContext(OperationType.CapsuleUpdate, scriptsRoot);
    ...
    [Fact]
    public void GetOperationCommand_IncludesCapFilePath_ForCapsuleUpdateWithCapFile()
    {
        var scriptsRoot = CreateScriptsRoot();
        var resolver = CreateResolver(scriptsRoot);
        var capFile = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid()}.cap");
        ...
        try
        {
            ...
            Assert.Contains(".cap", command.ArgumentsTemplate, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("--host", command.ArgumentsTemplate);
            Assert.Contains("--thor-host", command.ArgumentsTemplate);
        }
        finally
        {
            File.Delete(capFile);
    ...
    [Fact]
    public void GetOperationCommand_OmitsFilePath_ForCapsuleUpdateWithBinFile()
    {
        var scriptsRoot = CreateScriptsRoot();
        var resolver = CreateResolver(scriptsRoot);
        var context = CreateContext(OperationType.CapsuleUpdate, scriptsRoot);
        ...
        Assert.Contains("--host", command.ArgumentsTemplate);
        Assert.Contains("--thor-host", command.ArgumentsTemplate);
        Assert.DoesNotContain(".bin", command.ArgumentsTemplate, StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static ThorScriptResolver CreateResolver(string scriptsRoot)
    {
        return new ThorScriptResolver(
            ...
            new ScriptExecutionSettings
            {
                ...
                CapsuleArgumentsTemplate = "--host {hostIp} --thor-host {targetIp}"
            });
    }

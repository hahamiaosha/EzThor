...
public sealed class EnvironmentProfileSerializationTests
{
    ...
    public void EnvironmentProfile_RoundTripsThroughJson()
    {
        var profile = new EnvironmentProfile
        {
            ...
            LastFlashMode = FlashMode.Uefi,
            LastFlashSlot = FlashSlot.A,
        ...
        Assert.Equal(profile.HostUser, roundTripped.HostUser);
        Assert.Equal(profile.TargetUser, roundTripped.TargetUser);
        Assert.Equal(profile.LastFilePath, roundTripped.LastFilePath);
        Assert.Equal(profile.LastOperation, roundTripped.LastOperation);
        Assert.Equal(profile.LastFlashMode, roundTripped.LastFlashMode);
        Assert.Equal(profile.LastFlashSlot, roundTripped.LastFlashSlot);

...
public sealed class ThorScriptResolver : IThorScriptResolver
{
    ...
    public ScriptCommandDefinition GetOperationCommand(OperationContext context)
    {
        ...
        return context.OperationType switch
        {
            ...
            OperationType.CapsuleUpdate => BuildCapsuleUpdateCommand(context),
            _ => throw new InvalidOperationException($"Unsupported operation type '{context.OperationType}'.")
        };
    }

    private ScriptCommandDefinition BuildCapsuleUpdateCommand(OperationContext context)
    {
        var scriptPath = ResolveScriptPath(context.ScriptsRootPath, _thorScriptSettings.CapsuleScriptRelativePath);

        // For .cap files, append the file path so the script sends the local capsule directly.
        // For .bin files, omit it — send_build.sh has already generated the capsule on the build host.
        var template = IsCapsuleFile(context.FilePath)
            ? $"{_scriptExecutionSettings.CapsuleArgumentsTemplate} {{filePath}}"
            : _scriptExecutionSettings.CapsuleArgumentsTemplate;

        var scriptArguments = ShellTemplateExpander.ExpandScriptArguments(template, context);
        return CreateShellDefinition(scriptPath, scriptArguments, context.WorkingDirectory);
    }

    private static bool IsCapsuleFile(string? filePath)
    {
        return !string.IsNullOrWhiteSpace(filePath)
               && Path.GetExtension(filePath).Equals(".cap", StringComparison.OrdinalIgnoreCase);
    }
